...
public class TutorialEnemySpawner : MonoBehaviour
{
    ...
    [SerializeField]
    private TutorialEnemyAI enemyPrefab;
    ...
    void Spawn()
    {
        ...
        TutorialEnemyAI ai = Instantiate(enemyPrefab, spawnPath.startPosition.position, Quaternion.identity);
        ai.transform.SetParent(this.transform);
        ai.transform.LookAt(player);
        ai.gameObject.name = "TutorialEnemy";
        ...
        curSpawnIndex += 1;
        spawnActive = false;
        player.GetComponentInChildren<PlayerController>().tutorialEnemy = ai;
